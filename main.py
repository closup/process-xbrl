...
def write_html(input_file : str,
    ...
    for sheet_name in input_xl.sheet_names:
        if not(sheet_name in ["Label Dropdowns", "Master Info"]):
            sheets.append(Sheet(input_file, sheet_name, context_name_map)) 
    acfr = Acfr(sheets)
...
def upload_file(output_file = "static/output/output.html", format = "gray"):
    ...
    write_html(excel_files[0], output_file, context_name_map, format)
...
def successful_upload():
    ...
    return "success"
        # if file in SPREADSHEET_EXTENSIONS:
